...
Original file is located at
    https://colab.research.google.com/drive/1pN1qQ-IszNPhDISktTLEV78ISaq4iPXu
"""
...
hist = model.fit(train_batches, validation_data=validation_batches, epochs = 10, verbose=1)
...
hist1 = model1.fit(x=train_batches, validation_data = validation_batches, epochs = 10, verbose =1)
...
model2.add(layers.Conv2D(filters=6, kernel_size=(3, 3), activation='tanh', input_shape=(32,32,1)))
...
hist2 = model2.fit(train_batches, validation_data=validation_batches, epochs = 10, verbose=1)

pd.DataFrame(hist2.history).plot(figsize = (10,6))
plt.show()

prediction2 = model2.predict(x=test_batches, verbose=1)

cm = confusion_matrix(y_true=test_batches.classes, y_pred = np.argmax(prediction2, axis=-1))
