...
Original file is located at
    https://colab.research.google.com/drive/13ScxizgqJIGVqlv9JfXYwu1vV9o9tDZ4
...
model = Sequential()
for layer in model_vgg.layers[:-2]:
  model.add(layer)
...
hist = model.fit(x=train_batches, validation_data = validation_batches, epochs = 15, verbose =1, callbacks=[callbacks] )

# exp
hist = model.fit(x=train_batches, validation_data = validation_batches, epochs = 15, verbose =1, callbacks=[callbacks] )

pd.DataFrame(hist.history)[['loss','val_loss']].plot(figsize=(10,6))
plt.show()

pd.DataFrame(hist.history)[['accuracy','val_accuracy']].plot(figsize=(10,6))
plt.show()
...
model.save_weights('weight_vgg16_full_trained', save_format='h5')
...
for i in batch[0][:16]:
  ...
  plt.subplot(4,4,j)
  plt.axis('off')
  plt.title('Pred='+str(pred[j])+' | Actual='+ str(batch[1][j]), color = 'g')
...
#   plt.subplot(4,4,j)
#   plt.axis('off')
#   plt.title('Pred='+ 'ok' if pred[j][0] == 1. else 'def'+
#             '| Actual=+'+ 'ok' if (batch[1][j][0]) ==1 else'def', color = 'g')
#   # plt.text(0,0,j,fontsize =10)
#   plt.imshow(img)
#   j+=1